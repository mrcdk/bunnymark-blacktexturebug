...
class BitmapTest extends Sprite 
{
	...
	public function new() 
	{
		...
		//bunnyAsset = Assets.getBitmapData("assets/wabbit_alpha.png", false);
		pirate = new Bitmap(Assets.getBitmapData("assets/pirate.png", false), PixelSnapping.AUTO, true);
		...
		for (i in 0...numBunnies) 
		{
			...
			var bitmap;
			if(Math.random() > 0.5)
				bitmap = new Bitmap (Assets.getBitmapData("assets/wabbit_alpha.png", false));
			else {
				var data:BitmapData = new BitmapData(148, 64, true);
				data.copyPixels(Assets.getBitmapData("assets/pirate.png", false), new Rectangle(0, 20 + Std.int(Math.random() * 70), 148, 64), new Point());
